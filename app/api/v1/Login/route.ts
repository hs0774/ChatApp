...
const zodschema = z.object({
    userDetail: z.string().min(2, "Name is required").max(50).refine(value => {
        return validator.isEmail(value) || (typeof value === 'string' && value.length <= 50);
...
export async function POST(req:Request,res:Response) {
    try {
     ...
     if(!validation.success) {
        ...
        if (!user || !(await bcrypt.compare(sanitizedData.password as string, user.password))) {
