...
import {
  ...
  wallZodSchema,
  commentZodSchema,
} from "../../../utils/helperFunctions/zodSchemas.ts";
...
io.on("connection", (socket) => {
  ...
  socket.on("create-comment", async ({ imageURL, comment, wallId, token }) => {
    ...
    // await wall.save();
    const commentToClient = {
      //new comment object to be added to the specified post
      _id: comment._id,
      sender: {
        _id: user?.id,
        username: user?.username,
    ...
    io.to(`wall_${decodedToken.id}`).emit(
      "create-comment",
      wallId,
      commentToClient,
      user?.id
  ...
  socket.on("delete-comment", async ({ wallId, commentId, token }) => {
    ...
    const isOwnerOrCommenter =
      wall.user.equals(user._id.toString()) ||
      wall.replies.some((reply) => reply.sender.equals(user._id.toString()));

    if (isOwnerOrCommenter) {
      wall.replies = wall.replies.filter(
  ...
  socket.on("toggle-like", async ({ action, wallId, token }) => {
    ...
    if (action === "add") {
      ...
      const hasLiked = wall.likes.some(
        (id) => id.toString() === user._id.toString()
      );
      if (!hasLiked) {
        return;
      }
      const newWall = wall.likes.filter(
        (id) => id.toString() !== user._id.toString()
      );
      wall.likes = newWall;
      await wall.save();
