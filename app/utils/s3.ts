...
const migrateImages = async () => {
  try {
    await dbConnect();
    let count = 0;
    const imageFilenames = fs.readdirSync(path.join(__dirname, '../../public/sampleImgs'));
    // replace User Profile Pictures
    const users = await User.find({ profilePic: { $exists: true, $ne: null } });
    for (const user of users) {
      const filePath = path.join(__dirname, '../../public/sampleImgs', imageFilenames[count]);
      const s3Url = await uploadToS3(filePath, 'profilePics', user._id.toString());
    ...
    for (const wall of walls) {
      const filePath = path.join(__dirname, '../../public/sampleImgs', imageFilenames[count]);
      const s3Url = await uploadToS3(filePath, 'wallImages', wall._id.toString());
      wall.image = s3Url;
      count++;
      ...
      for (const reply of wall.replies) {
        if (reply.image) {
          const filePath = path.join(__dirname, '../../public/sampleImgs', imageFilenames[count]);
          const replyS3Url = await uploadToS3(filePath, 'replyImages', reply._id.toString());
