...
export default function Profile({ params }: ProfileParams) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        setPosts(userData.user.wall.reverse());
        setEditDetails((prev) => ({
          ...prev,
  ...
  }, [username, user?.id, router]); //because of next naming conventions this is the id
  ...
  async function goToChat(username: string, id: string) {
    ...
    const existingChat = data?.user.chats.some(
      (chat: Chat) =>
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      localStorage.setItem(
        "profilePic",
        responseData.newlyEditedData.profilePic
      );
  ...
  return (
    <div>
      {!editing ? (
        <div className="pfpAndDetails">
          {data?.user.profilePic && (<Image
          ...
          <ul className="dropdown-menu">            
            ...
            {/* <h4> */}
            <br />
              <li><strong>Age:</strong> {data?.user.details.age}</li>
              
              <li><strong>Bio:</strong> {data?.user.details.bio}</li>
              <li><strong>Hobbies:</strong> {data?.user.details.hobbies.join(", ")}</li>
              <li><strong>Occupation:</strong> {data?.user.details.job}</li>
              <li><strong>Location:</strong> {data?.user.details.location}</li>
              <li><strong>Sex:</strong> {data?.user.details.sex}</li>
            {/* </h4> */}
          </ul>
        </div>
      ) : (
        ...
        <div className="dropdown-me">
          ...
          data?.user.friends.some(
            (friend: Friend) => friend._id === user?.id
          ) || data?.user.nonFriendsChat) && (
            <button
              onClick={() => goToChat(data?.user.username, data?.user._id)}
            >
              Chat
            </button>
          )}
          ...
          {user?.id !== data?.user._id &&
            (data?.user.friends.some(
              ...
              <>
                ...
                <button onClick={() => goToInbox(data?.user.username)}>
                  Send inbox
                </button>
              </>
            ) : (
            ...
            <div className="friendLists">
              ...
              <div className="friendsListsLeft">
                <h2>Friends list </h2>
                <ul>
                    ...
                    <li key={friend.username} className="namePlusDropdown">
                      ...
                      {user?.id === data?.user._id && (
                        <div className="dropdownHolder">
                          ...
                          >
                            <button
                              onClick={() => goToChat(friend.username, friend._id)}
                            >
                              Chat
                            </button>
                            <button onClick={() => goToInbox(friend.username)}>
                              Inbox
                            </button>
              ...
              {(user?.id === data?.user._id && (data?.suggestedFriends?.length ?? 0) > 0) && (
                <div className="friendsListRight">
                  <h2>Suggested Friends</h2>
                  {data?.suggestedFriends?.map((sFriend) => (
                    <Link key={sFriend._id} href={`/profile/${sFriend._id}`}>
                      <div className="picAndNamee dropdown-menu">
                        <div className="picccc">
                          <Image
                            className="friendsProfilePic"
                            src={sFriend.profilePic || ""}
                            height={20}
                            width={20}
                            alt={`${sFriend.username}'s profile picture`}
                          />
                          <p>
                            {sFriend.username} 
                          </p>
