...
export default function Profile({ params }: ProfileParams) {
    ...
    function friendSetting(event: MouseEvent<HTMLButtonElement, MouseEvent>): void {
        //so here we want to open a tiny modal with three options to go the the
        //persons page, chat with them or defriend them 
        console.log('do this once friend and user profile frontends plus inbox page is up')
    }

    function goToInbox(event: MouseEvent<HTMLButtonElement, MouseEvent>): void {
   ...
   async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    return (
        <div>
            ...
            <>
                <ul>
                    {user?.id === data?.filteredUser._id && <button onClick={detailsEdit}>Edit</button>}
            ...
            <form onSubmit={handleSubmit}>
                <div>
                    ...
                    <input type="text" id="username" name="username" value={editDetails.username} onChange={handleChange} />
                </div>
                <div>
                    <label htmlFor="age">Age:</label>
                    <input type="number" id="age" name="age" value={editDetails.age} onChange={handleChange} />
                </div>
                <div>
                    <label htmlFor="bio">Bio:</label>
                    <textarea id="bio" name="bio" value={editDetails.bio} onChange={handleChange}></textarea>
                </div>
                <div>
                    <label htmlFor="job">Occupation:</label>
                    <input type="text" id="job" name="job" value={editDetails.occupation} onChange={handleChange} />
                </div>
                <div>
                <select id="location" name="location" value={editDetails.location} onChange={handleChange}>
                ...
                <div>
                    ...
                    <select id="sex" name="sex" value={editDetails.sex} onChange={handleChange}>
            ...
            <div>
                {user?.id !== data?.filteredUser._id && (
                    data?.filteredUser.nonFriendsChat && <button onClick={goToChat}>Chat</button>
                )}
                ...
                {user?.id !== data?.filteredUser._id && (
                    data?.filteredUser.friends.includes(user?.id) ? (
                        <>
                            ...
                            <p>Friends <button onClick={friendSetting}>Remove Friend</button></p> 
                            <button onClick={goToInbox}>Send inbox</button>
                        </>
                        
                    ) : (
                        <>
                        {data?.status === 'pending'? <button>Requested</button> :
                            ...
                            {user?.id === data?.filteredUser._id && (
                                <div className="dropdown">
                                    ...
                                    <div id={`dropdown-${friend.username}`} className="dropdown-menu">
                                        ...
                                        <button onClick={friendSetting}>Remove Friend</button>
                                    </div>
                                </div>
                            )}
                        </li>
