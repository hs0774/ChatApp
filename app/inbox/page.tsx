...
export default function Inbox() {
    ...
    async function handleDelete() {
        try {
            ...
            const messagesToDelete = messages?.message
                .filter(message => document.getElementById(`message-${message._id}`)?.checked)
            ...
            const updatedMessages = messages?.message.filter(message => !document.getElementById(`message-${message._id}`)?.checked);
    ...
    function handleDeleteAll(event) {
        ...
        const checkboxes = document.querySelectorAll('input[type="checkbox"][name^="message-"]');
    ...
    return (
        <div>
            <div className="inboxCommands">
                <h2>Message List</h2>
                <button onClick={() => { setOpenInboxMessage(false); setOpenMessage(true) }}>Create Message</button>
            ...
            <div className="inboxAndMessage">
                <div className="inboxLeftSide">
                    ...
                    <input type="checkbox" name="deleteAll" onChange={handleDeleteAll} checked={deleteAllChecked} />
                            ...
                            <div>
                                <li className="inboxMessages">
                                    <div className="inboxLeft">
                                        <input type="checkbox" id={`message-${message._id}`} name={`message-${message._id}`} onChange={checkIfUnchecked} />
                                    </div>
                ...
                <div className="inboxRightSide">
                    {openMessage && <CreateMessage friends={friends} />}
