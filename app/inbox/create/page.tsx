...
export default function CreateMessage({friends}) {
        ...
        <div className="nameSearch">
          ...
          <div className="dropdown">
            ...
            friends.filter((item) => {
              ...
              return (searchTerm && arrayItemName.startsWith(searchTerm) && arrayItemName !== searchTerm);
            }).slice(0, 10).map((item) => (
              <div className="dropdown-row" key={item._id} onClick={()=>setFormData(prev => ({...prev,receiver:item.username}))}>{item.username}</div>
            ))}
          </div>
        </div>
